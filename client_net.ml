...
let input_ipv4 ~iface ~router dns_client packet =
...
let add_vif { Dao.ClientVif.domid; device_id } dns_client ~client_ip ~router ~cleanup_tasks qubesDB =
  ...
  Netback.listen backend ~header_size:Ethernet_wire.sizeof_ethernet (fun frame ->
    ...
    | Ok (eth, payload) ->
        ...
        | `IPv4 -> input_ipv4 ~iface ~router dns_client payload
        | `IPv6 -> return () (* TODO: oh no! *)
  )
  >|= or_raise "Listen on client interface" Netback.pp_error

(** A new client VM has been found in XenStore. Find its interface and connect to it. *)
let add_client dns_client ~router vif client_ip qubesDB =
  let cleanup_tasks = Cleanup.create () in
  Log.info (fun f -> f "add client vif %a with IP %a"
               Dao.ClientVif.pp vif Ipaddr.V4.pp client_ip);
  Lwt.async (fun () ->
      Lwt.catch (fun () ->
          add_vif vif dns_client ~client_ip ~router ~cleanup_tasks qubesDB
        )
        (fun ex ->
...
let listen dns_client qubesDB router =
  Dao.watch_clients (fun new_set ->
    ...
    new_set |> Dao.VifMap.iter (fun key ip_addr ->
      if not (Dao.VifMap.mem key !clients) then (
        let cleanup = add_client dns_client ~router key ip_addr qubesDB in
